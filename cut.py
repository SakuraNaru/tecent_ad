...
def cut_feature():
    # os.chdir(r'preliminary_contest_data/userFeature.data')
    ix = 0

    fo =  open('preliminary_contest_data/cut_user/userFeature%s.csv' %ix, 'w')
...
def cut_train():
    df_train = pd.read_csv('preliminary_contest_data/train.csv')
    ...
    for index,row in df_train.iterrows():
        ...
        if ix%400000  == 0:
            print(ix)
            df_part.to_csv('preliminary_contest_data/cut_train/train%s.csv' %ix, index=False)
            df_part = pd.DataFrame()


def mkSubFile(lines, head, srcName, sub):
    [des_filename, extname] = os.path.splitext(srcName)
    path=des_filename.split('/')
    filename =  path[0]+"/cut_train/"+path[1]+ '_' + str(sub) + extname
...
if __name__ == '__main__':
    cut_feature()
    # splitByLineCount('preliminary_contest_data/train.csv', 400000)
